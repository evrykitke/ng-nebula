...
import { AppTone, NAV_ITEMS, NavItem, apps, filterNav } from '../../core/layout/nav.model';
...
/**
 * Workspace → Apps: every module the user can open, as a launcher.
 *
...
@Component({
  ...
  template: `
    <app-page-header
      ...
      [breadcrumbs]="[{ label: 'Workspace' }, { label: 'Apps' }]"
    />
    ...
    @if (available().length === 0) {
      ...
      <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        @for (app of available(); track app.label) {
          <button
            type="button"
            class="group flex items-start gap-3.5 rounded-xl border border-border bg-card p-4 text-left
          ...
          >
            <span
              ...
              [class]="toneClass(app)"
            >
              <ng-icon [name]="app.icon ?? 'lucideLayoutGrid'" size="20" />
            </span>
            <span class="min-w-0 flex-1">
              <span class="block text-sm font-semibold text-foreground">{{ app.label }}</span>
...
export class AppsPage {
  ...
  readonly available = computed(() =>
    apps(filterNav(NAV_ITEMS, (names) => this.auth.hasAnyPermission(names))),
  );
  ...
  /** The first few pages, so a card says what the app is rather than only its name. */
  ...
  open(app: NavItem): void {
    ...
    const first = (app.children ?? []).find((c) => c.route)?.route;
