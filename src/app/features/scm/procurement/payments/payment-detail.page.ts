...
import { fmtDate, fmtDateTime, fmtMoney, statusLabel } from '../../shared/scm-format';
import { PaymentView, ProcurementServiceProxy } from '../../../../shared/service-proxies/service-proxies';
...
export class PaymentDetailPage {
  ...
  readonly canReverse = computed(() => this.auth.hasPermission(Permissions.paymentsReverse));
  ...
  private load(): void {
    ...
    this.error.set(null);
    this.proxy.get_payment(this.id).subscribe({
      next: (p) => {
        this.payment.set(p);
  ...
  post(): void {
    if (this.busy()) return;
    this.busy.set(true);
    this.proxy.post_payment(this.id).subscribe({
      next: (p) => {
        this.busy.set(false);
        this.payment.set(p);
        this.notify.success('Payment posted to the ledger');
      },
      error: (err) => {
        this.busy.set(false);
  ...
  async remove(): Promise<void> {
    ...
    if (!ok) return;
    this.busy.set(true);
    this.proxy.delete_payment(this.id).subscribe({
      next: () => {
        this.busy.set(false);
        this.notify.success('Draft deleted');
        void this.router.navigate(['/procurement/payments']);
      },
      error: (err) => {
        this.busy.set(false);
  ...
  confirmReverse(): void {
    if (this.busy()) return;
    this.busy.set(true);
    this.proxy.reverse_payment(this.id, { reason: this.reverseReason.trim() || undefined }).subscribe({
      next: (p) => {
        this.busy.set(false);
